...
- [x] Реализовать функцию `parse_channel_subscribers()` для получения списка участников канала через Pyrogram
- [x] Добавить обработку ошибок (права доступа, ограничения API, сетевые ошибки)
- [x] Реализовать поэтапный парсинг с задержками для больших каналов (ограничения Telegram API)
- [x] Добавить логгирование процесса парсинга
- [x] Реализовать фильтрацию ботов и неактивных пользователей
- [x] Добавить сбор статистики (с username/без username, активность)

### 1.2. Расширить `database/database.py`
- [x] Добавить функцию `bulk_add_channel_subscribers()` для массового добавления подписчиков
- [x] Реализовать функцию `get_channel_subscribers_stats()` для получения статистики по каналу
- [x] Добавить функцию `clear_channel_subscribers()` для очистки данных подписчиков канала
- [x] Реализовать batch-вставку для оптимизации производительности
- [x] Добавить функцию `update_existing_subscribers()` для обновления существующих записей

### 1.3. Тесты для Этапа 1
- [x] Написать тесты для `channel_parser.py` с моками Pyrogram (`tests/test_channel_parser.py`)
...
- [x] Добавить состояние `AdminStates.parsing_channel`
- [x] Добавить состояние для хранения промежуточных данных парсинга
- [x] Добавить состояния для отслеживания прогресса парсинга
...
- [x] Добавить кнопку "Начать парсинг подписчиков"
- [x] Добавить клавиатуру статуса парсинга с прогрессом
- [x] Добавить кнопку "Отменить парсинг"
- [x] Реализовать клавиатуру с результатами парсинга

### 2.4. Обновить тексты в `texts/messages.py`
- [x] Добавить тексты для сообщений о парсинге
- [x] Добавить тексты для статистики парсинга
- [x] Добавить тексты ошибок парсинга
- [x] Добавить тексты прогресса парсинга
