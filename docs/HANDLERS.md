...
- **Обработка активности пользователей:**
  - Обработка комментариев в группе обсуждения канала и обновление активности пользователей
  - Обработка реакций на посты в канале
  - Обновление активности пользователя при взаимодействии

**Особенности работы:**
- Сохраняет подписчиков в базу данных, но не добавляет их в участники розыгрыша
- Использует функции `add_channel_subscriber`, `remove_channel_subscriber` и `update_last_activity` из `database.database` для управления состоянием подписки и активности
...
- **Создание розыгрыша:**
  ...
  - Ввод персонального сообщения для победителей (`message_winner`)
...
- **Управление розыгрышами:**
  - Редактирование полей розыгрыша (заголовок, описание, медиа, время, сообщение для победителей)
...
- **Дополнительная функциональность:**
  ...
  - Хранение и использование персонального сообщения для победителей из поля `message_winner` розыгрыша
  - Поддержка HTML-разметки в сообщениях для победителей
  - Форматирование подтверждения с превью сообщения для победителей
...
Каждый файл содержит функцию `setup_<module>_handlers(dp: Dispatcher)`, которая регистрирует соответствующие хендлеры в диспетчере. Основная регистрация происходит в `main.py` через вызов `setup_handlers(dp)` из `handlers/__init__.py`.
