...
- `get_winers_keyboard(winer_data)` - клавиатура с ссылками на победителей розыгрыша для быстрого доступа

Особенности:

- Некоторые клавиатуры асинхронные (например, `get_winers_keyboard`), так как могут требовать дополнительной обработки данных
- Клавиатура `get_winers_keyboard` генерирует кнопки с deep linking-ссылками на пользователей Telegram
